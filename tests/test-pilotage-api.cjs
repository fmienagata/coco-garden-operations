...
(async () => {
 ...
 try {
  ...
  assert.equal((await action({action:'pay',method:'cash',confirmed:false})).status,400);
  assert.equal((await action({action:'pay',method:'cash',orderId:String(foreignId)})).status,404);
  assert.equal((await action({action:'refund',reason:'Test remboursement'})).status,409);
  assert.equal((await action({action:'pay',method:'invalid'})).status,400);
  const concurrent = await Promise.all([action({action:'pay',method:'cash'}),action({action:'pay',method:'cash'})]);
  assert.deepEqual(concurrent.map(r=>r.status).sort(),[200,409]);
  assert.equal((await action({action:'collect'})).status,200);
  assert.equal((await action({action:'collect'})).status,409);
  assert.equal((await action({action:'refund',reason:'Recette du module'})).status,200);
  assert.equal((await action({action:'refund',reason:'Recette du module'})).status,409);
  const day=localDay();
  ...
  assert.ok(row); assert.equal(row.status,'collected'); assert.equal(row.settlement.amount,1200); assert.ok(row.settlement.refundedAt); assert.equal(demo.rows.some(r=>r._id===String(foreignId)),false);
  ...
  assert.equal((await fetch(base+'/api/orders/update',{method:'POST',headers,body:JSON.stringify({id:String(id),patch:{total:1}})})).status,409);
  const kitchen = await (await fetch(base+'/api/orders',{headers})).json(); assert.equal(kitchen.find(r=>r._id===String(id)).status,'collected');
